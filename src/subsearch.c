...
// extern int tcp_sock;
int _subsearch_subcount;
char** _subsearch_subbuff;
// int pagenumber; /* Used as a variable to store current page number (of results)*/
char _subsearch_pagestring[32]; /* The string to request a new page */
...
int get_subdomains(char* host)
{
    ...
    engine = 1;
    while (1) {
        memset(_subsearch_pagestring, '\0', sizeof(_subsearch_pagestring));
        memset(sendbuff, '\0', sizeof(sendbuff));
        switch (engine) {
        case 1:
            if (!pagenumber)
                print_line("Searching Google.com:80...\n");
            ...
            snprintf(_subsearch_pagestring, sizeof(_subsearch_pagestring), "rt=%d&sa=N", (pagenumber + 1) * 100);
            break;
        case 2:
            if (!pagenumber)
                print_line("Searching Altavista.com:80...\n");
            ...
            snprintf(_subsearch_pagestring, sizeof(_subsearch_pagestring), "&stq=%d", (pagenumber + 1) * 100);
            break;
            /*			case 3:
                                            ...
                                            snprintf(_subsearch_pagestring, sizeof(_subsearch_pagestring), ">Next >");
                                            break; */
        default:
            break;
        }
        sendData(sendbuff, strlen(sendbuff));
        if (get_hoststring((char*)hostwww))
            pagenumber++;
        else {
            if (engine == 2)
                break;
    ...
    if (tcp_sock)
        tcp_sockdcon();
    /* Clean up */
    if (_subsearch_subbuff)
        for (i = 0; i < _subsearch_subcount; i++)
            free(_subsearch_subbuff[i]);
    if (_subsearch_subcount == -1)
        _subsearch_subcount = 0;
    memset(sendbuff, '\0', sizeof(sendbuff));
    snprintf(sendbuff, sizeof(sendbuff), "Found %d possible subdomain(s) for host %s, Searched %d pages containing %d results\n", _subsearch_subcount, hostwww, totalpages, totalpages * 100);
...
int get_hoststring(char* host)
{
    ...
    do {
        ...
        if (strstr(readbuff, _subsearch_pagestring))
            newpage = 1;
        if (strstr(readbuff, host)) {
            ckpos = 0;
            for (crpos = 0; readbuff[crpos + 1] != '\0' && crpos < (int)strlen(readbuff); crpos++) {
                if (readbuff[crpos] == '<' && readbuff[crpos + 1] == '/' && readbuff[crpos + 2] == 'b' && readbuff[crpos + 3] == '>' && (crpos + 5) < (int)strlen(readbuff))
                    crpos += 4;
                if (readbuff[crpos] == '<' && readbuff[crpos + 1] == 'b' && readbuff[crpos + 2] == '>' && (crpos + 4) < (int)strlen(readbuff))
                    crpos += 3;
                if (readbuff[crpos] == host[ckpos])
                    ckpos++;
                else
                    ckpos = 0;
                if (ckpos == (int)strlen(host) && readbuff[(crpos - ckpos) - 10] == '.') {
                    if (grab_sub(readbuff, (crpos - (ckpos + 11)), host))
                        return 0;
                    ckpos = 0;
                }
                if (ckpos == (int)strlen(host) && readbuff[crpos - ckpos] == '.') {
                    if (grab_sub(readbuff, (crpos - (ckpos + 1)), host))
                        return 0;
                    ckpos = 0;
                }
            }
...
int grab_sub(char* buff, int pointer, char* host)
{
    ...
    while (pointer >= 0 && ctr < NCOL && (buff[pointer] == '-' || buff[pointer] == 46 || (buff[pointer] >= 48 && buff[pointer] <= 57) || (buff[pointer] >= 65 && buff[pointer] <= 90) || (buff[pointer] >= 97 && buff[pointer] <= 122))) {

        /* Sometimes strings can contain %3Dhost.com %3D being an equals sign, sometimes seen in
           php query strings etc. */

        if (buff[pointer] == 'D' && buff[pointer - 1] == '3' && buff[pointer - 2] == '%')
            break;

        flippedsub[ctr] = buff[pointer];

        if (buff[pointer] == '>' && buff[pointer - 1] == 'b' && buff[pointer - 2] == '/' && buff[pointer - 3] == '<')
            pointer -= 4;
        if (buff[pointer] == '>' && buff[pointer - 1] == 'b' && buff[pointer - 2] == '<')
            pointer -= 3;
        ctr++;
        pointer--;
    }
    ...
    if (!strlen(sub) || strlen(sub) > NCOL || sub[0] == '.')
        return 0;
    if (sublist(sub, host))
        return 1;
...
int sublist(char* sub, char* host)
{
    ...
    for (i = 0; i < _subsearch_subcount && _subsearch_subcount > 0; i++) {
        if (!strcmp(_subsearch_subbuff[i], sub)) {
            return 0;
        }
    }

    snprintf(output, sizeof(output), "%s.%s", sub, host);
    if (!get_host(output, host_ip))
        return 0;
    print_line("HostName:%s\n", output);
    print_line("HostIP:%s\n", host_ip);
    _subsearch_subbuff = realloc(_subsearch_subbuff, (_subsearch_subcount + 1) * (sizeof(char) * NCOL));
    if (!_subsearch_subbuff) {
        print_line("ERROR: Realloc in SubSearch module failed\n");
        return 1;
    }
    _subsearch_subbuff[_subsearch_subcount] = malloc((NCOL + 1) * sizeof(char));
    if (!_subsearch_subbuff[_subsearch_subcount])
        print_line("ERROR: Malloc in SubSearch module failed\n");

    strcpy(_subsearch_subbuff[_subsearch_subcount], sub);
    _subsearch_subcount++;
    return 0;
}
