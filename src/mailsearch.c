...
// extern int tcp_sock;
int _mailsearch_emailcount;
char** _mailsearch_emailbuff;
...
int get_emails(char* host)
{
    ...
    while (1) {
        ...
        if (get_email_hoststring((char*)hostwww))
            pagenumber++;
        else {
            if (engine == 2)
                break;
    ...
    if (tcp_sock)
        tcp_sockdcon();
    /* Clean up */
    if (_mailsearch_emailbuff)
        for (int i = 0; i < _mailsearch_emailcount; i++)
            free(_mailsearch_emailbuff[i]);

    if (_mailsearch_emailcount == -1)
        _mailsearch_emailcount = 0;

    memset(sendbuff, '\0', sizeof(sendbuff));
    snprintf(sendbuff, sizeof(sendbuff), "Found %d E-Mail(s) for host %s, Searched %d pages containing %d results\n", _mailsearch_emailcount, hostwww, totalpages, totalpages * 100);
        ...
        if (strstr(readbuff, host)) {
            ...
            for (crpos = 0; readbuff[crpos + 1] != '\0' && crpos < (int)strlen(readbuff); crpos++) {
                ...
                if (ckpos == (int)strlen(host) && (readbuff[(crpos - ckpos) - 10] == '@' || readbuff[(crpos - ckpos) - 10] == '.')) {
                    if (grab_emails(readbuff, (crpos - (ckpos + 10)), host))
                        return 0;
                    ckpos = 0;
                }
                if (ckpos == (int)strlen(host) && (readbuff[crpos - ckpos] == '@' || readbuff[crpos - ckpos] == '.')) {
                    if (grab_emails(readbuff, (crpos - ckpos), host))
                        return 0;
                    ckpos = 0;
                }
            }
...
int grab_emails(char* buff, int pointer, char* host)
{
    ...
    while (pointer >= 0 && ctr < NCOL && (buff[pointer] == '-' || buff[pointer] == 64 || buff[pointer] == 46 || (buff[pointer] >= 48 && buff[pointer] <= 57) || (buff[pointer] >= 65 && buff[pointer] <= 90) || (buff[pointer] >= 97 && buff[pointer] <= 122))) {
    ...
    if (strlen(flippedemail) >= NCOL)
        return 0;

    for (ctr = (int)strlen(flippedemail); ctr >= 0; ctr--) {
        email[strlen(email)] = flippedemail[ctr];
    }
    if (!strlen(email) || strlen(email) >= NCOL || email[0] == '@' || !strstr(email, "@"))
        return 0;
    if (emaillist(email, host))
        return 1;
...
int emaillist(char* email, char* host)
{
    ...
    for (int i = 0; i < _mailsearch_emailcount && _mailsearch_emailcount > 0; i++) {
        if (!strcmp(_mailsearch_emailbuff[i], email)) {
            return 0;
        }
    }

    _mailsearch_emailbuff = realloc(_mailsearch_emailbuff, (_mailsearch_emailcount + 1) * (sizeof(char) * NCOL));
    if (!_mailsearch_emailbuff) {
        print_line("ERROR: Realloc in E-MailSearch module failed\n");
        return 1;
    }

    _mailsearch_emailbuff[_mailsearch_emailcount] = malloc(NCOL * sizeof(char));
    if (!_mailsearch_emailbuff[_mailsearch_emailcount])
        print_line("ERROR: Malloc in E-MailSearch module failed\n");

    strcpy(_mailsearch_emailbuff[_mailsearch_emailcount], email);

    snprintf(output, sizeof(output), "%s%s\n", _mailsearch_emailbuff[_mailsearch_emailcount], host);
    print_line("%s", output);

    _mailsearch_emailcount++;
    return 0;
}
